/**
 * Pick a colourway for a site the agent is looking at.
 *
...
import { COLLECTIONS, PRESETS, presetName } from "liquidforge/presets"
import type { LiquidPreset, MaterialFamily } from "liquidforge/presets"
...
export function recommend(input: RecommendInput): Recommendation {
  ...
  const target = input.brandColor ? oklab(input.brandColor) : null
  const scored = ids
    .map((id) => PRESETS[id])
